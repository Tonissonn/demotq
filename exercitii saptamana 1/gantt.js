let input = [
  ...
  let allInputDates = input.map(a=>a.date)
  let minDate = allInputDates.reduce(function (a, b) { return a < b ? a : b; }); 
  ...
  let endYear = new Date(maxDate).getFullYear()
  ...
  for(let year=startYear;year<=endYear;++year){
    let minMonth = 0, maxMonth=11, leapYear = year%4;
    let newYear={};
  
    if(year === startYear) minMonth = startMonth
    if(year === endYear) maxMonth = endMonth
    ...
    for(let month = minMonth; month<=maxMonth; ++month){
        ...
        newMonth["from"]=[]
        newMonth["list"]={}
        if(leapYear === 0){newMonth[29]=29;
            noMailsSent = "-".repeat(monthsLen[month]+1);
        }
        else{
            noMailsSent = "-".repeat(monthsLen[month])
        }
        
        newMonth["noMails"] = noMailsSent
  ...
  for(let year=startYear; year<= endYear; ++year){
    ...
    for(let month = minMonth; month<=maxMonth; ++month){
        let length = timeline[year][month]["noMails"]
        let calendar = ""
        for(let digit=1; digit<=length.length; ++digit){
        ...
        timeline[year][month]["from"].forEach(a=>{
          let tabs = "\t"
          if(a.length<16) tabs = "\t\t" 
          console.log(a+tabs+timeline[year][month]["list"][a])
        })
        console.log("*************************************************************")
    }
  }